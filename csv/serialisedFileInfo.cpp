...
void CSV::serializeFiles(const std::unordered_map<absFilePath, std::unique_ptr<middleend::MiddleEndState::file_info>>& files, std::filesystem::path csvPath) {

	std::ofstream outputCsv{ csvPath , std::ios::openmode::_S_trunc | std::ios::openmode::_S_out };

	outputCsv << "RealPath,WasEverCreated,WasEverDeleted,IsCurrentlyOnTheDisk,WasInitiallyOnTheDisk,FileType,AccessedAs" << std::endl;
...
std::unordered_map<absFilePath, std::unique_ptr<middleend::MiddleEndState::file_info>> CSV::deSerializeFiles(std::filesystem::path csvPath) {
	using namespace csv;
	CSVFormat format{};
	format.delimiter(',').quote('"').header_row(0).variable_columns(VariableColumnPolicy::THROW);
	CSVFormat accessformat{};
	accessformat.delimiter(',').quote('"').column_names({"path","dir","exec","flags"}).variable_columns(VariableColumnPolicy::THROW);
	...
	for (auto& row : inputCSV) {
		...
		if (!accessCSV.empty()) {
			...
			for (auto& row : accessParse)
			{
				acesses.emplace(
					-1,
					do_parse<decltype(middleend::access_info::relPath)>(row["path"]),
					do_parse<decltype(middleend::access_info::flags) >(row["flags"]),
					do_parse<decltype(middleend::access_info::executable)>(row["exec"]),
					do_parse<decltype(middleend::access_info::workdir)>(row["dir"])
				);

			}
		}

		results.try_emplace(realPath,
			std::make_unique<middleend::MiddleEndState::file_info>(
				...
				do_parse<decltype(FI::isCurrentlyOnTheDisk)>(row["IsCurrentlyOnTheDisk"]),
				do_parse<decltype(FI::wasInitiallyOnTheDisk)>(row["WasInitiallyOnTheDisk"]),
				optTransform(do_parse<std::optional<size_t>>(row["FileType"]), [](size_t v) {return static_cast<decltype(FI::type)::value_type>(v); }),
				false
			)
		);
	}
...
std::vector<std::string> CSV::deSerializeEnv(std::filesystem::path csvPath)
{
	...
	format.delimiter(',').quote('"').header_row(-1).variable_columns(csv::VariableColumnPolicy::KEEP);
...
void CSV::serializeEnv(const std::vector<std::string>& env, std::filesystem::path csvPath)
{
	std::ofstream res{csvPath, std::ios::openmode::_S_trunc | std::ios::openmode::_S_out };
...
void CSV::serializeWorkdir(const std::filesystem::path& path, std::filesystem::path csvPath) {
	std::ofstream output(csvPath, std::ios::openmode::_S_trunc | std::ios::openmode::_S_out);
	output << path.string();
}
