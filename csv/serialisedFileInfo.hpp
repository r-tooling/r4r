...
namespace CSV {
	...
	inline void write(std::ostream& out, std::string_view str, bool writeComma = true) {
		if (str.find(",") != str.npos || str.find("\n") != str.npos || str.find("\"") != str.npos) {
			out << "\"" << replaceAll < std::string, std::string_view> (std::string{ str }, "\"", "\"\"\"") << "\"";
		}
		else {
			out << str;
		}
	...
	inline void write(std::ostream& out, const std::optional<Contained>& str, bool writeComma = true) {
		if (str.has_value()) {
	...
	void serializeFiles(
		const std::unordered_map<absFilePath, std::unique_ptr<middleend::MiddleEndState::file_info>>& files,
		std::filesystem::path csvPath);


	std::unordered_map<absFilePath, std::unique_ptr<middleend::MiddleEndState::file_info>> 
		deSerializeFiles(std::filesystem::path csvPath);

	std::vector<std::string> deSerializeEnv(std::filesystem::path csvPath);
	void serializeEnv(const std::vector<std::string>& env, std::filesystem::path csvPath);
	
	std::filesystem::path deSerializeWorkdir(std::filesystem::path csvPath);
	void serializeWorkdir(const std::filesystem::path& path, std::filesystem::path csvPath);
}