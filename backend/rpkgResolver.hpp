...
struct RpkgPackage {
    ...
    RpkgPackage(std::u8string packageName, std::u8string packageVersion,
                std::u8string rVersion,
                std::unordered_set<std::u8string> dependsOn,
                std::filesystem::path whereLocated, bool isBaseRpackage)
        : packageName{packageName}, packageVersion{packageVersion},
          rVersion{rVersion}, dependsOn{dependsOn}, whereLocated{whereLocated},
          isBaseRpackage{isBaseRpackage} {}

    explicit operator std::u8string() const noexcept {
        std::u8string result =
            packageName + u8"-" + packageVersion + u8": " + rVersion;
        return result;
    }
...
using RpkgSet =
    std::unordered_set<RpkgPackage, Hasher<&RpkgPackage::packageName>,
...
struct TopSortIterator {
    ...
    auto begin() { return iterator{items}; }
    ...
    struct EndSentinel {};
    struct iterator {
        std::unordered_set<ExpendedRpkg, Hasher<&ExpendedRpkg::package>,
                           Compare<&ExpendedRpkg::package>>
            toIterateOver; // owns data identified by package*
        std::unordered_set<const ExpendedRpkg*>
            dependsOnNone; // points to owned data.
        ...
        iterator(const RpkgSet& items) noexcept {
            ...
            for (const auto& package : items) {
                for (const auto& dependingOn : package.dependsOn) {
                    ...
                    toIterateOver.find(&package)->thisDependsOn.emplace(
                        depOnPtr);
            ...
            if (dependsOnNone.empty() && !toIterateOver.empty()) {
                fprintf(stderr,
                        "Cycle detected during topsorting of R packages. "
                        "Tagging a random package as having no dependencies. "
                        "May cause more warning later on \n");
        ...
        const iterator& operator++() noexcept {
            auto* removingItem =
                *dependsOnNone
                     .begin(); // if this fails the iterator was invalid
            for (auto* depOnMe : removingItem->dependsOnMe) {
                if (auto pkg = toIterateOver.find(depOnMe);
                    pkg != toIterateOver.end() &&
                    pkg->thisDependsOn.erase(removingItem->package) == 1) {
        ...
        const RpkgPackage& operator*() noexcept {
            ...
            return *(*it)->package; // for a valid iterator this has to be
                                    // valid.
        }
...
struct Rpkg {
  public:
    ...
    // TopSortIterator should not have access to the internals of rpkg-based
    // lookups.
    std::unordered_map<std::filesystem::path, std::optional<const RpkgPackage*>>
        resolvedPaths;

    std::optional<const RpkgPackage*>
    resolvePathToPackage(const std::filesystem::path&);
    ...
    void persist(std::ostream& dockerImage,
