...
struct Apt {
    using AptInfoMap =
        std::unordered_set<AptInfo, Hasher<&AptInfo::mangledPackageRepoName>,
                           Compare<&AptInfo::mangledPackageRepoName>>;

    Apt();
    AptLookupInfo resolveNameToSourceRepo(const std::u8string& packageName);
    // items are never invalidated. this is depended upon elsewhere in the
    // project, retain.
    const AptInfo&
    translatePackageToIdentify(const std::u8string& packageRepoMangledName);

    const AptInfoMap& encounteredRepositories() { return repoInstalLookup; }
