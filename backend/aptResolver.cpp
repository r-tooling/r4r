...
std::vector<std::u8string> listAllRepos() {
    ...
    for (auto const& dir_entry :
         std::filesystem::directory_iterator{"/etc/apt/sources.list.d/"})
        str.emplace_back(dir_entry.path());

    // grep "^[^#]" / etc / apt / sources.list / etc / apt / sources.list.d/*
    auto dpkgProcess = spawnStdoutReadProcess(
        "grep", ArgvWrapper{"-h", "^[^#]", "/etc/apt/sources.list", str},
...
backend::AptLookupInfo
backend::Apt::resolveNameToSourceRepo(const std::u8string& name) {
    using std::operator""sv;
    ...
    auto dpkgProcess =
        spawnStdoutReadProcess("apt-cache", ArgvWrapper{"policy", nameSV},
                               []() noexcept { setlocale(LC_ALL, "C.UTF-8"); });
    ...
    for (auto buffer : LineIterator{dpkgProcess.out.get()}) {
        ...
        case sourceLookup:
            ...
            if (packageRepo !=
                u8"/var/lib/dpkg/status") { // this is a lcoal thing, retry
                                            // required
                state = done;
            } else {
                state = anotherSourceORVersionLookup;
                fprintf(stderr,
                        "Version of %s seems to be unacessible in any package. "
                        "Attempting recovery.\n",
                        toNormal(name).c_str());
                /*´todo: what do here? I assume fallback to the older version
and warn libpcre2-8-0: Installed: 10.40-1+ubuntu20.04.1+deb.sury.org+1
...
500 http://security.ubuntu.com/ubuntu jammy-security/main amd64 Packages
        ...
        case anotherSourceORVersionLookup:
            if (data.find(u8"http") != data.npos) { // is a link
                ...
                fprintf(stderr,
                        "Error finding repo for installed version of package "
                        "%s = %s \n",
...
const backend::AptInfo&
backend::Apt::translatePackageToIdentify(const std::u8string& packageRepo) {

    if (auto it = repoInstalLookup.find(packageRepo);
        it != repoInstalLookup.end()) {
        return *it;
    }

    std::u8string translatedPackage;
    // https://cloud.r-project.org/bin/linux/ubuntu jammy-cran40/ Packages ->
    ...
    // if we were with a higher c++ version I'd ipplay range transforms here
    // with splits but alas.
    //(.*)/[^ ](.*) => \1 \2
    while (!data.empty() && data != u8"") {
        ...
        if (offset == data.npos) {
            ...
            break;
        } else if (offset + 1 < data.size() &&
                   data[offset + 1] != u8' ') { // has a character after the /
    ...
    return *repoInstalLookup.emplace(packageRepo, u8"deb" + translatedPackage)
                .first;
}
