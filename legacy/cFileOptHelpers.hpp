...
/*
        Unique-ptr like class for FILE* constructed from a file descriptor - and
...
struct ToBeClosedFileFD : protected ToBeClosedFile {
    ...
    explicit ToBeClosedFileFD(ToBeClosedFd fd, const char* mode) noexcept(
        noexcept(ToBeClosedFile(fdopen(fd.release(), mode))))
        : ToBeClosedFile(fd ? fdopen(fd.get(), mode) : nullptr),
          underlyingFD(fd.release()) {
    } // technically I shall be releasing into the parent call but I need to
      // store the get value here...

    explicit operator fileDescriptor() { return underlyingFD; }
    constexpr ToBeClosedFile::value_type release() noexcept {
        underlyingFD = -1;
        return ToBeClosedFile::release();
    }

    void reset(ToBeClosedFile::value_type other =
                   ToBeClosedFile::del::invalidValue) noexcept {
        tryClose();
        fd = other;
        if (fd) {
    ...
    constexpr ToBeClosedFile::value_type get() const noexcept { return fd; }
...
struct freeDeleter {
    static_assert(std::is_trivially_destructible_v<T>,
                  "Override this, if you really want the underlying class to "
                  "be freed and not have desrtuctor called.");
...
struct OpaqueUniquePtr : public ToBeClosedGeneric<T*, freeDeleter<T>> {
    ...
    T*& getRef() { return this->fd; }
};
/*
A helper iterator for iterating over a FILE* line by line. Returns the buffer as
a string_view into the buffer.
*/
struct LineIterator {
    FILE* file;
    ...
    struct IteratorImpl {
        ...
        bool operator==(const EndSentinel&) const { return nrRead < 0; }
        bool operator!=(const EndSentinel&) const { return nrRead >= 0; }
    ...
    auto begin() { return IteratorImpl{file}; }
};
