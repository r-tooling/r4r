...
namespace detail {
/*
        Helper function to be used as a decltype argument, easier to understand
   than a typedef.
*/
...
/*
        Used for creating a mutable zero terminated char[] from immutable const
   char*
...
template <class Type>
concept stringView_OrContainerOfStringView =
    std::is_convertible_v<Type, std::string_view> ||
    ContainerContainingSV<Type>;
/*
 * Overloads for getting the number of string views in a generic argument
 */
template <std::convertible_to<std::string_view> Item>
constexpr size_t ItemsCount(Item&&) {
    return 1;
};
template <ContainerContainingSV Item>
constexpr size_t ItemsCount(Item&& item) {
    return item.size();
};
/*
        How many string views does a list of string views or containers of
   string views contain?
*/
template <stringView_OrContainerOfStringView... Types>
constexpr size_t CountAll(Types&&... params) {
    return (0 + ... + ItemsCount(params));
};
/*
...
/*
        Take a list of just about any argument which could then be turned into a
   string view or a range of string views. Turn those into a mutable char*
   array.
*/
template <stringView_OrContainerOfStringView... Types>
inline std::unique_ptr<std::unique_ptr<char[]>[]>
fromConstCharArrayToCharPtrArray(Types&&... params) {
    size_t argc = CountAll(params...);
    auto buffer =
        std::make_unique_for_overwrite<std::unique_ptr<char[]>[]>(argc);
    auto ptr = buffer.get();
...
/*
 * Used for specifying arguments of a system call in an ergonomic way. Does not
 * preppend the program name Allocates new memory for the wrapped arguments. Can
 * be created with just about anything convertible to string_view or a a range
 * of string_views.
 */
struct ArgvWrapper {
    ...
    char** get() noexcept { return uniqueToSimple.get(); }
...
inline ToBeClosedFd readClosedPipe() {
    ...
    assert(pipe_res != -1);
    close(pipefd[0]); // I will not be reading that, who knows what will happen
                      // to it though.
    (void)pipe_res;
...
call. If the exec fails, the child is not allowed to return and exits. The child
invokes the callback right before spawing with valid file descriptors marked for
close on exec - can be used to for example revert some of the automated
markings.
*/
template <class Callback>
inline pid_t spawnProcess(int inFD, int outFD, int errFD,
...
/*
 * Spawns a process with argc error handling and pre-pending the filename to the
 * argv array
 */
template <class Callback>
inline pid_t
spawnProcessWithSimpleArgs(int inFd, int outFD, int errFD,
                           const std::filesystem::path& programPath,
                           char* const argv[], size_t argc, Callback callback) {
    if (argc < 1) {
        argc = 0; // and implicitly argv should point to nullptr, but I will not
                  // be checking that, for all I care argv can be nullptr.
    }
    // setup args arrays.
    clone_args args{
    ...
    // this requires fork not clone be used. Otherwise we do not know when they
    // shall be freed.
    std::unique_ptr<char[]> filename = nullptr;
    {
        ...
        filename.get()[sv.size()] =
            0; // ensure null terminated, may end up with two null terminators,
               // better than 0
    }
...
spawnProcessWithSimpleArgs(int inFd, int outFD, int errFD,
                           const std::filesystem::path& programPath,
                           argv_t args, Callback callback) {
    return spawnProcessWithSimpleArgs(inFd, outFD, errFD, programPath,
                                      args.get(), args.argc, callback);
};
...
inline ReturnState waitForTermination(pid_t pid) {
    ...
    do {
        ...
        if (returnStatus == -1) {
            ...
            case ECHILD:
                fprintf(stderr,
                        "Waitpid The process specified by pid does not exist "
                        "or is not a child of the calling process, or the "
                        "process group specified by pid does not exist or does "
                        "not have any member process that is a child of the "
                        "calling process. \n");
                break;
            case EINTR:
                fprintf(stderr, "Waitpid The function was interrupted by a "
                                "signal. The value of the location pointed to "
            ...
            assert(returnStatus ==
                   pid); // this could change in the case of catching the wait
                         // of another pid. THIS SHOULD BE IMPOSSIBLE DUE TO THE
                         // NATURE OF THE CALL
        }
    } while (!WIFEXITED(status) && !WIFSIGNALED(status));

    if (WIFSIGNALED(status)) {
        // fprintf(stderr, "Direct child process terminated by signal with
        // status %d\n", WTERMSIG(status));
...
struct PidDeleter {
    ...
    constexpr bool is_valid(const pid_t& fd) const noexcept { return fd >= 0; }
};
/*
...
struct [[nodiscard]] SpawnedValues {
    toBeClosedPid pid; // ORDER MATTERS HERE! we first close the FDs and then
                       // wait for termination. Do not reorder unles defining
                       // your own destructor with explicit reset calls
    ...
    SpawnedValues(pid_t pid, ToBeClosedFd err, ToBeClosedFd out)
        : pid(pid), stderrFD(std::move(err)), stdoutFD(std::move(out)) {}
    SpawnedValues(pid_t pid, fileDescriptor err, fileDescriptor out)
        : pid(pid), stderrFD(std::move(err)), stdoutFD(std::move(out)) {}
};
struct ReturnStateWrapper {
    std::optional<ReturnState> processRes;
    bool terminatedCorrectly() {
        return processRes.has_value() &&
               std::holds_alternative<CodeReturn>(processRes.value()) &&
               std::get<CodeReturn>(processRes.value()).code == 0;
    }
};

/*
All the relevant parts of a spawned process wrapped in a class with the bonus of
using C-style FILE* It does not igev c++ streams as there is no method for
creating a stream from a FILE* nor from a file descriptor.
*/
struct [[nodiscard]] SpawnedValuesFilePtr {
    ...
    SpawnedValuesFilePtr(pid_t pid, fileDescriptor err, fileDescriptor out)
        : pid(pid), err(ToBeClosedFd{err}, "r"), out(ToBeClosedFd{out}, "r") {}
};

/*
 * A high level function for spawning a process with file descriptor returns.
 */
template <class Callback = decltype(detail::NULLOPTFUNCTION)*,
          ArgvWrapperLike argv_t>
inline SpawnedValues
spawnReadOnlyProcess(const std::filesystem::path& programPath, argv_t&& argv,
                     Callback&& callback = detail::NULLOPTFUNCTION) {
    int errfd[2];
...
/*
 * A high level function for spawning a process which is only meant to write to
 * stdout.
 */
template <class Callback = decltype(detail::NULLOPTFUNCTION)*,
          ArgvWrapperLike argv_t>
inline SpawnedValuesFilePtr
spawnStdoutReadProcess(const std::filesystem::path& programPath, argv_t&& argv,
                       Callback&& callback = detail::NULLOPTFUNCTION) {
...
template <class Callback = decltype(detail::NULLOPTFUNCTION)*,
          ArgvWrapperLike argv_t>
inline SpawnedValuesFilePtr
spawnStdoutStderrMergedProcess(const std::filesystem::path& programPath,
                               argv_t&& argv,
                               Callback&& callback = detail::NULLOPTFUNCTION) {
    int outfd[2];
    auto pipe_res = pipe(outfd);
    ...
    auto pipe = readClosedPipe();
    auto pid =
        spawnProcessWithSimpleArgs(pipe.get(), outfd[1], outfd[1], programPath,
                                   argv.get(), argv.argc, callback);
    close(outfd[1]);
    return {pid, -1, outfd[0]};
}
...
inline bool checkExecutableExists(const std::filesystem::path& programPath,
                                  argv_t&& argv) {
    static auto codeRet = [](CodeReturn c) { return c.code; };
    static auto signalRet = [](SignalReturn c) {
        return c.signal == SIGPIPE ? 0 : -1;
    }; // sigpipe is bound to hapen as I just close the output immadietely.
       // Could be improved to redirect to /dev/null.
    return optTransform(
               spawnStdoutReadProcess(programPath, argv).close().processRes,
               [](auto var) {
                   return std::visit(overloaded{codeRet, signalRet}, var);
               })
               .value_or(-1) == 0;
}
