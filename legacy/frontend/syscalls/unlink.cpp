...
void UnlinkBase::exit(processState& process, middleend::MiddleEndState& state,
                      long syscallRetval) {
    // fprintf(stderr, "unlink %s %ld\n", fileRelPath.c_str(), syscallRetval);
    if (syscallRetval == 0) {
        state.removeNonDirectory(process.pid, state.resolveToAbsoluteDeleted(
                                                  process.pid, fileRelPath));
    }
}

void RmdirBase::exit(processState& process, middleend::MiddleEndState& state,
                     long syscallRetval) {
    // fprintf(stderr, "rmdir %s %ld\n", fileRelPath.c_str(), syscallRetval);
    if (syscallRetval == 0) {
        state.removeDirectory(process.pid, state.resolveToAbsoluteDeleted(
                                               process.pid, fileRelPath));
    }
}

void Unlink::entry(processState& process, const middleend::MiddleEndState&,
                   long) {
    at = AT_FDCWD;
    fileRelPath = process.ptrToStr<relFilePath>(process.getSyscallParam<1>());
}

void Rmdir::entry(processState& process, const middleend::MiddleEndState&,
                  long) {
    at = AT_FDCWD;
    fileRelPath = process.ptrToStr<relFilePath>(process.getSyscallParam<1>());
}

void UnlinkAt::entry(processState& process, const middleend::MiddleEndState&,
                     long) {
...
void UnlinkAt::exit(processState& process, middleend::MiddleEndState& state,
                    long syscallRetval) {
...
void UnlinkAt::entryLog(const processState& process,
...
void PathAtHolder::entryLog(const processState& process,
                            const middleend::MiddleEndState& state,
                            long syscallNr) {
