...
static_assert(getBuild() == std::string_view{"x86_64"},
              "The clone interfaces may differ on different plaforms");
namespace frontend::SyscallHandlers {

void Clone::entry(processState& process, const middleend::MiddleEndState& state,
                  long syscallNr) {
    ...
    // TODO: clone may terminate later than the cild process did. What shall be
    // done in such a case?
    // TODO: this is very much platform specific
    flags =
        process
            .getSyscallParam<1>(); // TODO: won't this fail for fork and such?
    handleEntryState(process, state, syscallNr);
}

void Clone::handleEntryState(processState& process,
                             const middleend::MiddleEndState&, long) {
    process.blockedInClone = {std::nullopt,
                              flags}; // TODO: maybe just save &this?
}

void Clone::exit(processState& process, middleend::MiddleEndState& state,
                 long syscallRetval) {
    assert(process.blockedInClone); // can be safely removed later on.
    if (syscallRetval >= 0) {
        state.trackNewProcess(syscallRetval, process.pid,
                              !(process.blockedInClone->flags & CLONE_FILES),
                              process.blockedInClone->cloneChildPid,
                              process.blockedInClone->flags & CLONE_FS);
        if (fileDescriptorPtr && fileDescriptorPtr.value() != 0) {
            auto ptr =
                userPtrToOwnPtr<pid_t>(process.pid, fileDescriptorPtr.value());
            state.registerProcessFD(process.pid, syscallRetval, *ptr);
        }
    } else {
        assert(!process.blockedInClone ||
               !process.blockedInClone->cloneChildPid.has_value());
    }

    process.blockedInClone = std::nullopt;
}

void Clone::entryLog(const processState& process,
                     const middleend::MiddleEndState& state, long syscallNr) {
    simpleSyscallHandler_base::entryLog(process, state, syscallNr);
    strBuf << "()";
}

void Clone3::entry(processState& process,
                   const middleend::MiddleEndState& state, long syscallNr) {
    size_t size = process.getSyscallParam<2>();
    (void)size;
    assert(size == sizeof(clone_args));
    auto data =
        userPtrToOwnPtr<clone_args>(process.pid, process.getSyscallParam<1>());
    flags = data->flags & ~0xff;
    fileDescriptorPtr = data->pidfd;
    handleEntryState(process, state, syscallNr);
}

void Fork::entry(processState& process, const middleend::MiddleEndState& state,
                 long syscallNr) {
    /*
...
void VFork::entry(processState& process, const middleend::MiddleEndState& state,
                  long syscallNr) {
    /*	struct kernel_clone_args args = {
