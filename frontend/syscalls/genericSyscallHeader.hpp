...
namespace frontend::SyscallHandlers {
	...
	struct NullOptHandler :SyscallHandler {
		void entry(processState&, const middleend::MiddleEndState&, long) override {};
		void exit(processState&, middleend::MiddleEndState&, long) override {};

		void entryLog(const processState&, const middleend::MiddleEndState&, long) override {};
		void exitLog(const processState&, const middleend::MiddleEndState&, long) override {};
	};

	struct WarnHandler :NullOptHandler {
		int syscallNr = 0;
		void entry(processState&, const middleend::MiddleEndState&, long nr) override {
			syscallNr = nr;
		};
		void exit(processState&, middleend::MiddleEndState& state, long) override {
			std::string call = "The entered syscall ";//todo are these allocations needed
	...
	struct simpleSyscallHandler_base : virtual SyscallHandler {
		...
		void exitLog(const processState& process, const middleend::MiddleEndState&, long syscallRetval) override
		{
			auto str = strBuf.str();
		...
		void appendResolvedFilename(const processState& process, const middleend::MiddleEndState& state, int fd, std::stringstream& strBuf)
		{
			if (fd == AT_FDCWD) {
				strBuf << "AT_FDCWD";
			}
...
//C
	...
	process.syscallData = TemplatedSyscallHandler<1>::entry(process,state,syscallNr); break;
...
case SYS_read:
	...
	process.syscallNr = 2;
	assert(process.syscallData == nullptr);
	process.syscallData = TemplatedSyscallHandler<2>::entry(process,state,syscallNr); break;
