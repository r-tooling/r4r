...
#ifdef __INTELLISENSE__ // magic because the msvc intellisesnse breaks on
                        // includes of the gcc stdlib
namespace std {
...
struct BytesIterator {
  ...
  private:
    ...
    static constexpr unsigned char getByte(const T v,
                                           const std::size_t offset) {
        std::make_unsigned_t<T> real =
            std::bit_cast<std::make_unsigned_t<T>>(v);
        if constexpr (std::endian::native == std::endian::little) {
            real >>= offset * 8;
            return static_cast<unsigned char>(real &
                                              std::make_unsigned_t<T>{0xFF});
        } else {
  ...
  public:
    struct const_iterator {
        ...
        constexpr unsigned char operator*() { return getByte(val, offset); }
    ...
    constexpr const_iterator begin() const { return {val}; }
    constexpr const_iterator end() const { return {sentinel{}}; }
};

inline void logPtraceError(int err) {
    switch (err) {
    case EBUSY:
        printf("(i386 only) There was an error with allocating or freeing a "
               "debug register.");
        break;
    case EFAULT:
        printf("There was an attempt to read from or write to an invalid"
        ...
        break;
    case EINVAL:
        printf("An attempt was made to set an invalid option.");
        break;
    case EIO:
        printf("request is invalid, or an attempt was made to read from or"
        ...
        break;
    case EPERM:
        printf("he specified process cannot be traced. This could be"
...
// using PTRACE_PEEKDATA is very slow. And it results in  thousands of calls.
// Instead I querry for a bunch of values at once and hope that the string is
// not at a nasty boundary which would require more handling which is currently
// not present.
...
inline result userPtrToString(pid_t processPid, const long processPtr) {
    ...
    auto resizeBuffer = [&]() {
        ...
        for (size_t i = 0; i < bufferCount; i++) {
            ...
            remote[i].iov_base = reinterpret_cast<void*>(
                processPtr + ((offset + (i * readSize))));
    ...
    while (!zeroReched && readAll) {
        resizeBuffer();
        auto read_count = process_vm_readv(processPid, local, bufferCount,
                                           remote, bufferCount, 0);
        ...
        for (auto it = v.begin() + offset; it != end; ++it) {
            if (*it == 0) {
                zeroReched = true;
                v.erase(it, v.end()); // the zero is automagically appended in
                                      // the string
                break;
            }
        }
    ...
    // reallocations are unfortunatelly bound to happen here a lot. I need to
    // sloooowly parse the entire string utilll I get to the 0 byte. consider
    // process_vm_readv instead for reading larger blocks at once. Or any other
    // adress space-sharing mechanism.
    return {v.begin(), v.end()};
}

inline std::unique_ptr<unsigned char[]>
userPtrToOwnPtr(pid_t processPid, long ptr, size_t count) {
    iovec remote[1] = {
        iovec{.iov_base = reinterpret_cast<void*>(ptr), .iov_len = count}};
    iovec local[1];
    ...
    (void)read_count;
    assert(read_count == static_cast<ssize_t>(
                             count)); // "error reading X bytes from the tracee"
    return localPtr;
}
template <class pointsToStruct>
inline std::unique_ptr<pointsToStruct, decltype(std::free)*>
userPtrToOwnPtr(pid_t processPid, long ptr) {
...
// TODO: figure out how to make it possible to specify the array size in the
// return type.
