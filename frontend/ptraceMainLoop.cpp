			...
			void* signalToPass = 0;
			{
				...
				case SignalInfo::stop: //(traced child has trapped);
					...
					*/
						...
						if (getSyscallRetval(process.pid) != -ENOSYS) {//syscall entry TODO: this may be platform specific!!!!
		...
		* TODO: get this value consistently for threads as well.
		...
		if (!pidFD) { //error state
			...
			case EINVAL:
				...
				//TODO: this happens when a process is cloned with CLONE_THREAD it seems to me. Needs to be researched more. As if only the thread group leader could pass its fd over.
				break;
			case EMFILE:
