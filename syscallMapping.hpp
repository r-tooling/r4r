...
static_assert(getBuild() == std::string_view{ "x86_64" }, "This header shoudl only be included for the given architecture as otherwise the syscall # will not match.");


//decide if you want to hide  definition in a cpp file or if we try to make a constexpr variant work.
//as this include is here only for the implementation, not the API.
#include <unordered_map>

//these could be a constexpr variant of a constexpr map which gets hidden inside a if(constexpr) block.
