...
//no simple way to describe the deduction here, no way to describe the member ptr without member type
//Unless std::hash<Key> is a program-defined specialization, h(k1) will never throw an exception.  - TODO: detect otherwise
...
struct Compare<memberPtr> {
	...
	bool operator()(const MemberType& lhs, const MemberType& rhs) const noexcept {
		return lhs == rhs;
	}
	bool operator()(const MemberType& lhs, const ClassType& rhs) const noexcept {
